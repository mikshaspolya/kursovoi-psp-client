...
public class DepositOrderController {
    ...
    @FXML
    private ListView<DepositOrderDto> depositOrderListView;
    ...
    void initialize() throws JsonProcessingException {
        ...
        var response = messageSender.sendMessage(CommandType.GET_ALL_PENDING_DEPOSIT_ORDERS, request);
        ...
        depositOrderListView.getItems().addAll(list);
    }
    ...
    void showOrderButtonClicked(ActionEvent event) throws IOException {
        ...
        if (depositOrderListView.getSelectionModel().getSelectedIndex() <= -1) {
            AlertManager.showMessage("Выберите вклад");
            presenter.show(Form.DEPOSIT_ORDER);
        } else {
            showOrderButton.getScene().getWindow().hide();
            DepositOrderDto deposit = depositOrderListView.getSelectionModel().getSelectedItem();
            ShowDepositOrderController.setOrder(deposit);
            presenter.show(Form.SHOW_DEPOSIT_ORDER);
        }
    }
}
